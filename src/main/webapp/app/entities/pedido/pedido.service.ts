...
export class PedidoService {
  ...
  protected convertDateFromClient(pedido: IPedido): IPedido {
    const copy: IPedido = Object.assign({}, pedido, {
      instante: pedido.instante && pedido.instante.isValid() ? pedido.instante.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.instante = res.body.instante ? moment(res.body.instante) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((pedido: IPedido) => {
        pedido.instante = pedido.instante ? moment(pedido.instante) : undefined;
      });
    }
    return res;
  }
